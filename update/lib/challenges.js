...
module.exports = async function updateTopics() {
  const response = await axios.get('https://raw.githubusercontent.com/odileeds/emer2gent-data/master/challenges.json');
  const data = response.data;
  if(JSON.stringify(existing) != JSON.stringify(data)) {
    const fp = path.join(__dirname, '../../_data/challenges.json');
